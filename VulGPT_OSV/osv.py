...
# This allows running the script without manually exporting env vars.
try:
    ...
    # Non-fatal: the user may be providing env vars via shell/docker.
    pass
...
# Do not hard-code secrets; provide via environment (or a local .env loaded by your shell).
...
def neo4j_session(driver):
    """Create a Neo4j session, honoring NEO4J_DATABASE if set."""
    if NEO4J_DATABASE:
        return driver.session(database=NEO4J_DATABASE)
    return driver.session()

OSV_API_BASE_URL = (os.environ.get("OSV_API_BASE_URL") or "https://api.osv.dev").rstrip("/")
OSV_API_QUERY_URL = f"{OSV_API_BASE_URL}/v1/query"
OSV_API_TIMEOUT_SECONDS = int(os.environ.get("OSV_API_TIMEOUT_SECONDS") or "60")

# Ecosystem names are case-sensitive in OSV.
OSV_QUERY_ECOSYSTEMS = [
    e.strip() for e in (os.environ.get("OSV_QUERY_ECOSYSTEMS") or "npm,PyPI,Maven").split(",") if e.strip()
]
OSV_QUERY_PACKAGES_PER_ECOSYSTEM = int(os.environ.get("OSV_QUERY_PACKAGES_PER_ECOSYSTEM") or "10")

# Fallback seed packages (used only when Neo4j has no Package nodes yet)
OSV_FALLBACK_PACKAGES = {
    "npm": ["react", "lodash", "express", "axios"],
    "PyPI": ["django", "flask", "requests", "numpy", "pandas"],
}
BATCH_SIZE = 100
PROCESSING_BATCH_SIZE = 100
MAX_WORKERS = 8
MAX_RECORDS_PER_RUN = int(os.environ.get("MAX_RECORDS_PER_RUN") or "1000")  # Process this many records per run
REPO_BASE_DIR = "repos"
...
STARTING_REPO_BATCH_SIZE = int(os.environ.get("STARTING_REPO_BATCH_SIZE") or "50")  # Increased from 5 to 50
MAX_REPO_BATCH_SIZE = int(os.environ.get("MAX_REPO_BATCH_SIZE") or "100")  # Increased from 20 to 100
REPO_BATCH_SIZE_INCREMENT = int(os.environ.get("REPO_BATCH_SIZE_INCREMENT") or "5")  # Increased from 1 to 5
...
def setup_git_ssh():
    ...
    # Avoid interactive prompts (can hang automation on Windows/CI)
    os.environ.setdefault('GIT_TERMINAL_PROMPT', '0')
    os.environ.setdefault('GCM_INTERACTIVE', 'never')


def _normalize_git_ref(ref: str) -> str:
    """Normalize git ref strings into something checkout/fetch can understand.
    ...
    ref = str(ref).strip()
    if '->' in ref:
        # Take the right side of symbolic refs.
...
# OSV API FETCHING
# -------------------------
def _get_osv_next_page_token(response_json: dict):
    """Return the pagination token from an OSV API response, if present."""
    if not isinstance(response_json, dict):
        return None
    return (
        response_json.get("next_page_token")
        or response_json.get("nextPageToken")
        or response_json.get("next_pageToken")
    )


def _osv_query_payload(package_name: str, ecosystem: str, page_token=None) -> dict:
    payload = {
        "package": {
            "name": package_name,
            "ecosystem": ecosystem,
    ...
    if page_token:
        # The documented field name is page_token, but the OpenAPI spec uses pageToken.
...
def query_osv_api_for_package(package_name: str, ecosystem: str) -> list:
    """Fetch OSV Vulnerability objects for a package (all versions).

    Uses POST /v1/query with only the package object (no version), which returns
    all vulnerabilities for the package.
    """
    vulns: list = []
    page_token = None

    while True:
        payload = _osv_query_payload(package_name, ecosystem, page_token=page_token)
        response = requests.post(OSV_API_QUERY_URL, json=payload, timeout=OSV_API_TIMEOUT_SECONDS)
        response.raise_for_status()
        data = response.json() or {}

        page_vulns = data.get("vulns") or []
        if page_vulns:
            vulns.extend(page_vulns)

        page_token = _get_osv_next_page_token(data)
        if not page_token:
            break
...
def get_packages_to_query_from_neo4j(limit_per_ecosystem: int = OSV_QUERY_PACKAGES_PER_ECOSYSTEM):
    """Return a list of (ecosystem, package_name) to query from OSV.

    Prefers Package nodes already present in Neo4j (so updates are targeted to
    what's relevant for the graph). Falls back to a small seed list for a fresh
    database.
    """
    ...
    try:
        ...
        with neo4j_session(driver) as session:
            result = session.run(
                """
                MATCH (p:Package)
                WHERE p.ecosystem IN $ecosystems
                OPTIONAL MATCH (p)-[:AFFECTED_BY]->(v:Vulnerability)
                WITH p, count(v) AS vulnCount
                RETURN p.ecosystem AS ecosystem, p.name AS name, vulnCount
                ORDER BY vulnCount DESC, name ASC
                """,
                ecosystems=OSV_QUERY_ECOSYSTEMS,
            )

            per_ecosystem = defaultdict(int)
            for record in result:
                ...
                if per_ecosystem[ecosystem] >= limit_per_ecosystem:
                    continue

                packages.append((ecosystem, name))
                per_ecosystem[ecosystem] += 1
    except Exception as e:
        print(f"Warning: Unable to read packages from Neo4j for OSV API queries: {e}")
    finally:
        try:
            driver.close()
        except Exception:
    ...
    # Fallback: seed a few popular packages if the DB has no Package nodes yet.
    fallback: list[tuple[str, str]] = []
    for ecosystem, names in OSV_FALLBACK_PACKAGES.items():
        if OSV_QUERY_ECOSYSTEMS and ecosystem not in OSV_QUERY_ECOSYSTEMS:
            continue
        for name in names:
            fallback.append((ecosystem, name))
    return fallback


def fetch_vulnerabilities_from_osv_api(max_records: int = MAX_RECORDS_PER_RUN):
    """Fetch up to max_records vulnerabilities from OSV via API.

    This replaces the previous 'all.zip' download. It queries OSV for a limited
    set of packages (from Neo4j, or a small fallback list for a fresh DB).
    """
    packages = get_packages_to_query_from_neo4j()
    if not packages:
        print("No packages available to query from OSV API.")
        return []

    print(f"Querying OSV API for {len(packages)} packages...")
    ...
    for ecosystem, package_name in packages:
        try:
            package_vulns = query_osv_api_for_package(package_name, ecosystem)
        except Exception as e:
            print(f"Error querying OSV API for {ecosystem}:{package_name}: {e}")
            continue
        ...
        for v in package_vulns:
            ...
            seen_ids.add(vid)
            vulns.append(v)
            if len(vulns) >= max_records:
                print(f"Reached max_records={max_records}; stopping OSV API fetch.")
                return vulns
...
# -------------------------
# LEGACY ZIP INGESTION (no longer used by main)
# -------------------------
...
def extract_repo_info(vuln):
    ...
    def clean_github_url(url):
        ...
        if url.startswith('https://github.com/'):
            ...
            if len(parts) >= 2:
                # Exclude GitHub advisory pages which are not cloneable repositories.
                # Example: https://github.com/advisories/GHSA-xxxx
                if parts[0].lower() == 'advisories':
                    return None
...
def safe_checkout_version(repo_path, version):
    ...
    """
    normalized = _normalize_git_ref(version)
    if not normalized:
        normalized = version

    try:
        # Attempt to checkout the specified version
        print(f"Checking out version {normalized}...")
        subprocess.run(
            ["git", "-C", repo_path, "checkout", "--force", normalized],
            check=True, capture_output=True
        )
        return True
    except subprocess.CalledProcessError:
        print(f"Could not checkout version {normalized} for repository at {repo_path}")

        # For shallow clones, the ref might not exist locally. Fetch just what we need.
        try:
            subprocess.run(
                ["git", "-C", repo_path, "fetch", "--depth", "1", "origin", normalized],
                check=True, capture_output=True
            )
            subprocess.run(
        ...
        try:
            ...
            default_branch = subprocess.check_output(
                ...
                text=True
            ).strip().rsplit('/', maxsplit=1)[-1]  # Extract branch name
        ...
        for version in matched_versions:
            ...
            try:
                ...
                try:
                    ...
                    # Windows-friendly fallback: compute size in Python
                    total_bytes = 0
                    try:
...
def update_language_data_for_repos(repo_urls):
    ...
    with neo4j_session(driver) as session:
        ...
        for repo_url in repo_urls:
            result = session.run("""
                MATCH (r:Repository {url: $url})-[:HAS_VERSION]->(v:Version)
                WHERE v.language_json IS NULL OR v.language_json = '{}' OR v.primary_language IS NULL OR v.primary_language = 'Unknown'
...
def insert_all_vulnerabilities_into_neo4j(vuln_records, selected_versions, repo_data=None):
    ...
    with neo4j_session(driver) as session:
        ...
        if new_vuln_records:
            ...
            with tqdm(total=len(new_vuln_records), desc="Filtering vulnerability records") as pbar:
                for vuln in new_vuln_records:
                    ...
                    # Filter affected packages to only include selected versions (when a selection exists)
                    if selected_versions and "affected" in processed_vuln:
                        filtered_affected = []
        ...
        # Insert repository and version information (even when there are no new vulns)
        if repo_data:
            ...
            """)
            existing_repos = {record["url"] for record in result if record and record.get("url")}

            # Get existing version IDs
            result = session.run("""
                MATCH (v:Version) RETURN v.id as id
            """)
            existing_versions = {record["id"] for record in result if record and record.get("id")}

            def _primary_language(languages_dict: dict) -> str:
                if not isinstance(languages_dict, dict) or not languages_dict:
                    return "Unknown"
                return max(languages_dict.items(), key=lambda kv: kv[1])[0]
            ...
            with tqdm(total=len(repo_data), desc="Processing repositories", unit="repo") as pbar:
                for repo in repo_data:
                    ...
                    # Link repository to vulnerabilities (best-effort)
                    ...
                    for ver in versions or []:
                        ...
                        if not version:
                            continue
                        version = _normalize_git_ref(version) or version
                        version_id = f"{repo_url}@{version}"
                        languages = ver.get("languages") if isinstance(ver, dict) else None
                        languages_json = json.dumps(languages) if isinstance(languages, dict) else None
                        primary_lang = _primary_language(languages) if isinstance(languages, dict) else "Unknown"
                        size = ver.get("size") if isinstance(ver, dict) else None
                        ...
                        session.run(
                            ...
                            SET v.version = $version,
                                v.repo_url = $repo_url,
                                v.size = $size,
                                v.language_json = $languages_json,
                                v.primary_language = $primary_language,
                                v.language_count = $language_count,
                        ...
                        # Repository -> Version relationship
                        session.run(
                            """
                            MATCH (r:Repository {url: $url})
                            MATCH (v:Version {id: $id})
            ...
            # If we added repositories, attempt a language update pass for missing language fields
            repo_urls = [r.get("url") for r in repo_data if r.get("url")]
            if repo_urls:
...
def main():
    ...
    """
    # Register timeout handler (SIGALRM/alarm are Unix-only; Windows doesn't support them)
    sigalrm = getattr(signal, "SIGALRM", None)
    signal_alarm = getattr(signal, "alarm", None)
    if sigalrm is not None and callable(signal_alarm):
    ...
    try:
        ...
        if len(sys.argv) > 1 and sys.argv[1] == "--backfill-repos":
            # Populate Repository/Version nodes for vulnerabilities fetched from OSV,
            # without requiring new vulnerability inserts.
            print("Backfill mode: analyzing repositories and writing Repository/Version nodes...")

            all_vulnerabilities = fetch_vulnerabilities_from_osv_api(max_records=MAX_RECORDS_PER_RUN)
            if not all_vulnerabilities:
                print("No vulnerabilities fetched from OSV API; nothing to backfill.")
                return

            repo_vuln_map = group_by_repository(all_vulnerabilities)
            print(f"Found {len(repo_vuln_map)} unique repositories")

            # Determine what is actually missing in Neo4j (source of truth).
            driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD))
            try:
                with neo4j_session(driver) as session:
                    result = session.run("MATCH (r:Repository) RETURN r.url as url")
                    existing_repo_urls = {record["url"] for record in result if record and record.get("url")}
            finally:
                driver.close()

            missing_repos = [repo_url for repo_url in repo_vuln_map.keys() if repo_url not in existing_repo_urls]
            print(f"Repositories missing in Neo4j: {len(missing_repos)}")

            _, current_batch_size = load_repository_progress()
            print(f"Current repository batch size: {current_batch_size}")

            repo_batch = missing_repos[:current_batch_size]
            ...
            for repo_url in tqdm(repo_batch, desc="Analyzing repositories"):
                try:
                    ...
                    vuln_ids = [v.get("id") for v in vulns if isinstance(v, dict) and v.get("id")]

                    target_versions = set()
                    for vuln in vulns:
                        for affected in (vuln.get("affected", []) if isinstance(vuln, dict) else []):
            ...
            # Keep progress file as informational only; Neo4j contents are the source of truth.
            processed_repos, _ = load_repository_progress()
            processed_repos.update(repo_batch)
            save_repository_progress(processed_repos, current_batch_size)

            # Insert only repository/version data (vuln list can be empty)
            insert_all_vulnerabilities_into_neo4j([], set(), repo_data)
            return
        ...
        # Fetch vulnerabilities via the official OSV API (no ZIP download)
        all_vulnerabilities = fetch_vulnerabilities_from_osv_api(max_records=MAX_RECORDS_PER_RUN)

        # Filter to vulnerabilities we haven't inserted yet (avoids expensive work)
        existing_vuln_ids = get_existing_vuln_ids()
        new_vulnerabilities = [v for v in all_vulnerabilities if v.get("id") not in existing_vuln_ids]
        ...
        if not new_vulnerabilities:
            print("No new vulnerabilities found from OSV API queries.")
            update_all_language_data()
            return

        print(f"Fetched {len(all_vulnerabilities)} vulnerabilities from OSV API; {len(new_vulnerabilities)} are new")

        # Use greedy algorithm to select minimum set of versions
        try:
            # Set timeout for this operation (30 minutes)
            timeout_seconds = 1800
            if signal_alarm is not None:
                signal_alarm(timeout_seconds)
                minimum_version_vulns, selected_versions = greedy_minimum_version_set(new_vulnerabilities)
                signal_alarm(0)
            else:
                # Best-effort timeout for platforms without SIGALRM (e.g. Windows)
                import threading
                import _thread

                timed_out = False

                def _interrupt_main_after_timeout():
                    nonlocal timed_out
                    timed_out = True
                    _thread.interrupt_main()

                timer = threading.Timer(timeout_seconds, _interrupt_main_after_timeout)
                timer.daemon = True
                timer.start()
                try:
                    minimum_version_vulns, selected_versions = greedy_minimum_version_set(new_vulnerabilities)
            ...
            minimum_version_vulns = new_vulnerabilities
            selected_versions = set()  # Empty set means all versions will be included
        finally:
            if signal_alarm is not None:
                # Ensure no pending alarms remain.
                signal_alarm(0)
        ...
        # Summary statistics
        print("\nSummary:")
        print(f"- Vulnerabilities fetched from OSV API: {len(all_vulnerabilities)}")
        print(f"- New vulnerabilities processed: {len(new_vulnerabilities)}")
